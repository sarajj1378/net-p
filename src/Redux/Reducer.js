...
const initialState = {
  ...
  loggedInUser: null,
  comments: [],
  wrongCredStatus: false,
  wrongCredMsg: "",
}

const addCommentItem = (state, itemForAdd) => {
  const isExist = state.comments.find(item => item.id === itemForAdd.id)
  if (isExist) {
    return state.comments.map(item => {
      if (item.id === itemForAdd.id) {
        return { ...item, comment: [...item.comment, itemForAdd.comment] }
      }
      return item
    })
  }
  return [...state.comments, { ...itemForAdd, comment: [itemForAdd.comment] }]
}
...
const rootReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case CONSTANTS.ADD_COMMENT:
      return {
        ...state,
        comments: addCommentItem(state, action.payload),
      }
    case CONSTANTS.WRONG_CRED:
      return {
        ...state,
        wrongCredStatus: action.payload.status,
        wrongCredMsg: action.payload.message || '',
      }
    default:
      return state
  }
